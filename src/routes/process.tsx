...
export const Route = createFileRoute("/process")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Every stage of Ntarakwai honey: hive inspection, colony health, harvesting, processing and packaging at Gatab, quality control and nationwide delivery." },
      { property: "og:title", content: "Our Process — Ntarakwai Beekeeping" },
      { property: "og:description", content: "From hive inspection on Mt. Kulal to a sealed jar at your door." },
  ...
  return (
    <>
      <PageHero
        eyebrow="Our Process"
        image={apiary}
        title={<>Eight stages, and not one of them <em className="text-honey">hurried</em>.</>}
      ...
      <section className="bg-background py-24 md:py-32">
        <div className="container-luxe">
          <ol className="relative mx-auto max-w-4xl">
            ...
            {STAGES.map((s, i) => (
              <li key={s.n} className="reveal-slow relative mb-12 md:pl-24" style={{ transitionDelay: `${(i % 4) * 90}ms` }}>
                ...
                <h2 className="font-display text-3xl leading-tight text-charcoal md:text-4xl">{s.t}</h2>
                <p className="mt-3 max-w-2xl text-lg leading-relaxed text-muted-foreground">{s.b}</p>
              </li>
            ))}
          </ol>
        </div>
      ...
      <ParallaxBand image={production} alt="" speed={0.18} className="py-24 md:py-32">
        <SectionHead
          tone="light"
          eyebrow="Standards"
          title={<>What “raw” means <em className="text-honey">here</em>.</>}
          intro="Raw is a word that gets used loosely. These are the specific commitments behind ours."
        />
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {[
            { t: "No pasteurisation", b: "Heat is not used to force clarity or delay crystallisation. Granulation is allowed to happen naturally." },
            { t: "No additives", b: "Nothing is blended in — no sugar syrup, no thinners, no flavouring. Honey and nothing else." },
            { t: "Batch integrity", b: "Seasons taste different and we let them. Each batch reflects what Mt. Kulal was flowering at the time." },
          ].map((c, i) => (
            <div key={c.t} className="reveal-slow rounded-3xl border border-cream/15 bg-charcoal/55 p-8 backdrop-blur" style={{ transitionDelay: `${i * 110}ms` }}>
      ...
      <section className="relative overflow-hidden bg-secondary/50 py-24">
        <div className="container-luxe flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div className="reveal max-w-2xl">
            ...
            <p className="mt-3 text-muted-foreground">Raw honey, beeswax and gift sets, delivered anywhere in Kenya.</p>
          </div>
          <div className="reveal flex flex-wrap gap-3">
            <Link to="/shop" className="btn-honey"><ShoppingBag className="h-4 w-4" /> Shop Now</Link>
            <Link to="/products" className="btn-outline-honey">See the range <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
        <img src={honeycomb} alt="" className="pointer-events-none absolute -right-20 -top-24 hidden h-72 w-72 rounded-full object-cover opacity-15 lg:block" loading="lazy" />
      </section>
    </>
  );
}
