        ...
        <div className="container-luxe">
          <div className="mx-auto max-w-md rounded-3xl border border-border bg-card p-12 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Browse our collection and add a jar of pure honey.
            </p>
            <Link to="/shop/products" className="btn-honey mt-8 inline-flex">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_400px]">
          ...
          <div className="reveal space-y-4">
            {resolved.map(({ product, qty, lineTotal }) => (
              <div
                key={product.id}
                className="grid grid-cols-[88px_1fr_auto] items-center gap-4 rounded-3xl border border-border bg-card p-4 shadow-sm sm:grid-cols-[120px_1fr_auto_auto]"
              >
                <Link
                ...
                <div className="min-w-0">
                  ...
                  <Link
                    to="/shop/product/$slug"
                    params={{ slug: product.slug }}
                    className="font-display mt-0.5 block truncate text-lg text-charcoal hover:text-honey-deep"
                  >
                    {product.name}
                  </Link>
          ...
          <aside className="reveal h-fit rounded-3xl border border-border bg-card p-7 shadow-sm lg:sticky lg:top-28">
            ...
            <dl className="mt-6 space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <dd className="font-medium text-charcoal">{formatKES(subtotal)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Delivery (est.)</dt>
                <dd className="font-medium text-charcoal">{formatKES(delivery)}</dd>
              </div>
              <div className="flex justify-between">
            ...
            <div className="mt-5 flex items-center gap-2 rounded-2xl bg-secondary/70 p-3 text-xs text-muted-foreground">
              ...
              <span>
                Delivery available in Marsabit, Isiolo, Meru, Laikipia (Nanyuki), and Nairobi.
              </span>
            </div>
          </aside>
        </div>
