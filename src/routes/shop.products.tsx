  ...
  head: () => ({
    meta: [
      { title: "All Products: Ntarakwai Shop" },
      {
        name: "description",
        content:
          "Browse every honey and bee product available for online order: raw honey, organic, beeswax, propolis, pollen and gift sets.",
      },
      { property: "og:title", content: "Shop All Honey Products" },
      {
        property: "og:description",
        content: "Pure honey and bee products available for nationwide delivery in Kenya.",
      },
    ],
  }),
...
function ShopCatalog() {
  ...
  const list = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      out = out.filter(
        (p) => p.name.toLowerCase().includes(term) || p.shortDesc.toLowerCase().includes(term),
      );
    }
    switch (sort) {
      case "price-asc":
        ...
        <div className="reveal flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-xl text-muted-foreground">
              {list.length} {list.length === 1 ? "product" : "products"} · delivery in Marsabit,
              Isiolo, Meru, Laikipia (Nanyuki) and Nairobi
            </p>
          </div>
          <div className="flex w-full flex-wrap items-center gap-3 md:w-auto">
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={(e) =>
                  navigate({
                    search: { ...search, sort: e.target.value as NonNullable<typeof sort> },
                  })
                }
        ...
        {list.length === 0 ? (
          <div className="mt-16 rounded-3xl border border-dashed border-border bg-card py-20 text-center">
            ...
            <button
              ...
              className="btn-outline-honey mt-6 text-sm"
            >
              Reset filters
            </button>
          </div>
                ...
                <Link to="/shop/product/$slug" params={{ slug: p.slug }} className="block">
                  <div className="relative aspect-[4/5] overflow-hidden bg-beige">
                    ...
                    {p.stock < 20 && p.stock > 0 && (
                      <span className="absolute right-4 top-4 rounded-full bg-destructive/90 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-cream">
                        Low stock
                      </span>
                    )}
                  </div>
