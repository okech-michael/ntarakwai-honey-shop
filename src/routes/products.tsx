...
const STORIES = [
  {
    ...
    benefits: [
      ...
      "Natural enzymes and pollen traces",
      "Season-specific character",
    ],
    pairing:
      "Straight from the spoon, over warm flatbread, or stirred into black tea once it has stopped steaming.",
  },
  {
    img: pProcessed,
    ...
    benefits: [
      "Consistent pour and texture",
      "Hospitality-friendly formats",
      "Long, stable shelf life",
    ],
    pairing:
      "Breakfast service, baking, dressings and anywhere a kitchen needs honey to behave predictably.",
  },
  {
    img: pBeeswax,
    ...
    pairing:
      "Balms, salves, wood finishes, candles and the workbench of anyone who makes things by hand.",
  },
  {
    img: pPropolis,
    ...
    benefits: [
      "Concentrated hive resin",
      "Traditionally used for wellness",
      "Forest-sourced on Mt. Kulal",
    ],
  ...
  head: () => ({
    meta: [
      { title: "Our Products: Raw Honey, Beeswax, Propolis & Pollen | Ntarakwai" },
      {
        name: "description",
        content:
          "Raw Mt. Kulal honey, clear table honey, natural beeswax, propolis, bee pollen and gift sets, the story, benefits and pairings behind each product.",
      },
      {
        property: "og:title",
        content: "Ntarakwai Products: Raw honey and hive products from Mt. Kulal",
      },
      {
        property: "og:description",
        content: "The story behind every jar: origin, benefits and how to use it.",
      },
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            Everything here came off one <em className="text-honey">mountain</em>.
          </>
        }
      ...
      <section className="bg-background py-24 md:py-32">
        <div className="container-luxe">
          <SectionHead
            ...
            title={
              <>
                Six products, one <em className="text-honey-deep">origin</em>.
              </>
            }
          ...
          <div className="mt-20 space-y-24 md:space-y-32">
            {STORIES.map((p, i) => (
              <article key={p.title} className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
                <div
                  className={`reveal-slow overflow-hidden rounded-[2.5rem] shadow-[var(--shadow-luxe)] ${i % 2 ? "lg:order-2" : ""}`}
                >
                ...
                <div className={`reveal-slow ${i % 2 ? "lg:order-1" : ""}`}>
                  <span className="text-[11px] font-semibold uppercase tracking-[0.3em] text-honey-deep">
                    {p.kicker}
                  </span>
                  <h3 className="font-display mt-4 text-4xl leading-[1.05] text-charcoal md:text-5xl">
                    {p.title}
                  </h3>
                  ...
                  <div className="mt-8 flex flex-wrap gap-3">
                    <Link to="/shop/products" className="btn-honey text-sm">
                      <ShoppingBag className="h-4 w-4" /> Buy online
                    </Link>
                    <a href={BRAND.phoneHref} className="btn-outline-honey text-sm">
        ...
        <div className="container-luxe flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div className="reveal max-w-2xl">
            <h2 className="font-display text-3xl leading-tight text-charcoal md:text-4xl">
              Custom volumes, private labels or wholesale?
            </h2>
            <p className="mt-3 text-muted-foreground">
              We supply retailers, hotels and distributors, and we are glad to talk through formats
              and branding.
            </p>
          </div>
          <div className="reveal flex flex-wrap gap-3">
            <Link to="/wholesale" className="btn-honey">
              Wholesale <ArrowRight className="h-4 w-4" />
            </Link>
            <Link to="/process" className="btn-outline-honey">
              How it is made
            </Link>
          </div>
        </div>
