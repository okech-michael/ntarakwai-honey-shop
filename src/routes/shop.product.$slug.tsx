...
export const Route = createFileRoute("/shop/product/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.product.name}: Ntarakwai Shop` },
...
function ProductDetail() {
  ...
  const related = SHOP_PRODUCTS.filter(
    (p) => p.id !== activeProduct.id && p.category === activeProduct.category,
  ).slice(0, 4);
  ...
  return (
    <div className="bg-background pt-28 pb-20">
      <div className="container-luxe">
        <Link
          to="/shop/products"
          className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-honey-deep"
        >
          ...
          <div className="reveal">
            ...
            <div className="mt-4 grid grid-cols-4 gap-3">
              {[product.image, product.image, product.image, product.image].map((src, i) => (
                <div
                  key={i}
                  className={`aspect-square overflow-hidden rounded-2xl border-2 ${i === 0 ? "border-honey-deep" : "border-border"} bg-beige`}
                >
          ...
          <div className="reveal">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-honey-deep">
              ...
              {product.badge && (
                <>
                  ...
                  <span className="rounded-full bg-honey/20 px-2 py-0.5 text-honey-dark">
                    {product.badge}
                  </span>
                </>
              )}
            ...
            <div className="mt-3 flex items-center gap-2 text-honey">
              {Array.from({ length: 5 }).map((_, k) => (
                <Star
                  key={k}
                  className={`h-4 w-4 ${k < Math.round(averageRating) ? "fill-current" : "opacity-40"}`}
                />
              ))}
            ...
            <div className="mt-6 flex items-baseline gap-3">
              <span className="font-display text-4xl text-honey-deep">
                {formatKES(activeProduct.price)}
              </span>
              {activeProduct.oldPrice && (
                <span className="text-lg text-muted-foreground line-through">
                  {formatKES(activeProduct.oldPrice)}
                </span>
              )}
            ...
            <ul className="mt-6 grid gap-2 sm:grid-cols-2">
              {activeProduct.benefits.map((b: string) => (
                <li key={b} className="flex items-center gap-2 text-sm text-charcoal">
                  <span className="grid h-5 w-5 place-items-center rounded-full bg-honey text-charcoal">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-1 rounded-full border border-border bg-card p-1">
                ...
                <button
                  onClick={() => setQty((q) => Math.min(activeProduct.stock, q + 1))}
                  aria-label="Increase"
                  className="grid h-10 w-10 place-items-center rounded-full hover:bg-secondary"
                >
              ...
              <button
                ...
                className="btn-honey flex-1 sm:flex-none disabled:cursor-not-allowed disabled:opacity-60"
              >
                {added ? (
                  <>
              ...
              <Link
                to="/shop/cart"
                className="btn-outline-honey flex-1 sm:flex-none"
                onClick={() => add(activeProduct.id, qty)}
              >
                Buy Now
              </Link>
            </div>
            ...
            <div className="mt-3 text-xs text-muted-foreground">
              {activeProduct.stock > 20 ? (
                <span className="text-honey-deep font-medium">✓ In stock</span>
            ...
            <div className="mt-8 grid gap-3 rounded-3xl border border-border bg-card p-5 text-sm">
              <div className="flex items-center gap-3">
                <Truck className="h-5 w-5 text-honey-deep" />
                <span>
                  Delivery via <strong>Via Wells Fargo or your preferred parcel courier</strong>,
                  1–3 business days
                </span>
              </div>
              <div className="flex items-center gap-3">
                <ShieldCheck className="h-5 w-5 text-honey-deep" />
                <span>Lab-tested, traceable & 100% pure</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-16 rounded-[2rem] border border-border bg-card p-7 shadow-sm md:p-9">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Share what you loved about this product.
          </p>
          ...
          <form onSubmit={handleReviewSubmit} className="mt-7 grid gap-4 md:grid-cols-2">
            <input
              ...
              className="rounded-full border border-input bg-background px-4 py-3 text-sm"
            />
            <input
              value={reviewTitle}
            ...
            <textarea
              ...
              className="md:col-span-2 rounded-2xl border border-input bg-background px-4 py-3 text-sm"
            />
            <div className="flex items-center gap-3 md:col-span-2">
              ...
              <button
                ...
                className="ml-auto inline-flex items-center gap-2 rounded-full bg-charcoal px-4 py-2 text-sm font-semibold text-cream"
              >
                {submittingReview ? (
                  "Saving..."
                ) : (
            ...
            {reviewSuccess && (
              <p className="md:col-span-2 text-sm text-honey-deep">{reviewSuccess}</p>
            )}
          </form>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 text-honey">
                    {Array.from({ length: 5 }).map((_, index) => (
                      <Star
                        key={index}
                        className={`h-4 w-4 ${index < review.rating ? "fill-current" : "opacity-40"}`}
                      />
                    ))}
                  </div>
                </div>
            ...
            <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {related.map((p) => (
                <Link
                  ...
                  params={{ slug: p.slug }}
                  className="group overflow-hidden rounded-3xl border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-luxe)]"
                >
