...
export function SectionHead({
  ...
  return (
    ...
    >
      ...
      {intro && (
        <p className={`mt-5 text-lg leading-relaxed ${tone === "light" ? "text-cream/75" : "text-muted-foreground"}`}>
          {intro}
        </p>
      )}
    </div>
...
export function StatRow({ items }: { items: readonly { value: string; label: string; note?: string }[] }) {
  return (
    <div className="grid gap-px overflow-hidden rounded-3xl border border-cream/15 bg-cream/10 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((s, i) => (
        <div key={s.label} className="reveal-slow bg-charcoal/70 p-7 backdrop-blur" style={{ transitionDelay: `${i * 110}ms` }}>
